      ...
      <div className="relative mt-16">
        ...
        <ol className="grid grid-cols-1 gap-x-6 gap-y-10 sm:grid-cols-2 lg:grid-cols-6 lg:gap-x-3">
          {steps.map((s, i) => (
            ...
            >
              <div className="relative z-10 flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl border border-ink-900/10 bg-white shadow-[0_16px_38px_-26px_rgba(10,10,11,0.65)]">
                <span className="absolute -right-1.5 -top-1.5 flex h-6 w-6 items-center justify-center rounded-full bg-ink-900 text-[11px] font-bold text-white shadow-sm">
                  {i + 1}
                </span>
                <s.icon size={22} strokeWidth={1.9} className="text-iris-700" />
              </div>
              <div className="lg:mt-4">
                <h3 className="text-[14px] font-semibold text-ink-900">
                  {s.title}
                </h3>
                <p className="mt-1.5 text-[12.5px] leading-relaxed text-ink-600 lg:px-1">
                  {s.body}
                </p>
              </div>
            </motion.li>
